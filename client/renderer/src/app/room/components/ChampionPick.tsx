...
function PlayerCard(props: PlayerCardProps) {
    ...
    return <div style={style}
        className={`${props.isRight ? styles['player-card-right'] : styles['player-card']} flex px-8 w-[420px] py-4 items-center border-b  border-slate-800 ${props.isRight ? "flex-row-reverse" : ""} `} >
...
export default function ({ selfID, seats, remainingTime, finished, diceNumber, availableChampions, onChange, onRandom }: ChampionPickProps) {

    const self = seats?.find(x => x?.id === selfID);
    ...
    const percentage = remainingTime ? (remainingTime / 60) * 100 : 0;

    return <div className={styles.container} style={containerStyles}>
        <div>
            {!finished ? (
